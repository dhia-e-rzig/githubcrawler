...
with open("../JSON Files/cloning2progress.json", 'r+') as outfile:
...
repos_list = open("../CSV Files/oldies/repo-metadata-sortedBySize.csv", "r", newline='')
repos_dataframe = pandas.read_csv(repos_list, delimiter=',')

with open("../CSV Files/oldies/reposNotFound.csv", "r", newline='') as repos_list:
    notfound_dataframe = pandas.read_csv(repos_list, delimiter=',')
    for i in range (progress,len(notfound_dataframe.index)):
        df=notfound_dataframe.loc[i,["url"]]
        print(df)
        arr = str(df["url"]).strip().split("/")
        name = arr[3]+"/"+arr[4][:-4]
        df2 = repos_dataframe.loc[repos_dataframe["full_name"] == name]
        category= df2["Category"].values.tolist()[0]
        try :
            a = urllib.request.urlopen(df["url"])
        except Exception as e :
            with open("../CSV Files/oldies/reposNotFoundAgain.csv", 'a+') as outfile:
                outfile.write(str(e)+","+category+","+name+","+df["url"]+"\n")
                outfile.flush()
            with open("../JSON Files/cloning2progress.json", 'w+') as outfile:
