...
def codechurn():
    df=pd.read_excel('../Excel Files/Codechurn.xlsx',sheet_name=0)
    ...
    df_all=(df_tool.append(df_applied)).append(df_non)
...
def avg_issue_duration_devopstool_vs_non_devopstool():
    df_tool_classification = pd.read_csv('../CSV Files/ToolProjects_classification.csv')
    df_applied_classification = pd.read_csv('../CSV Files/AppliedProjects_classification.csv')
    df_noaiml_classification = pd.read_csv('../CSV Files/NoAIMLProjects_classification.csv')
    ...
    L=plt.legend(bbox_to_anchor=(0.5, -0.35) ,borderaxespad=0,loc='lower center')
    L.get_texts()[0].set_text('Project that do not have Test tool(s)')
    L.get_texts()[1].set_text('Project that have Test tool(s)')

    plt.tight_layout()

    plt.savefig('RQ3-IssueReport_Test.eps', format='eps')
    plt.savefig('RQ3-IssueReport_Test.jpg', format='jpg')
    plt.show()
